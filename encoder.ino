 ...
 * 
 * DEVICE TO MEASURE DISTANCE AND SPEED
 */


const int encoder_a = 3; // Pin 3 GREEN
const int encoder_b = 2; // Pin 2 WHITE
const int hysteresis = 5; //hysteresis 5 readings, sensitivity 2mm ((wheelDia*PI)/600)*hysteresis
const float wheelDia = 55.0;  //wheel diameter in mm
...
void displayUnits(long ticks) 
{
      ...
      Serial.print(float((float(ticks)/1000)*singleTick),2);
      Serial.print(" meters  |  ");
      Serial.print((ticks*singleTick/25.4/12),2);
      Serial.print(" feet  |  ");
